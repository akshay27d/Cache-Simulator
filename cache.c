...
int main(int argc, char* argv[])
{
  ...
  //Initialize cache
	...
	uint32_t offSize = log22(line);
	uint32_t indexSize = log22(sets);
	uint32_t tagSize = 32-(indexSize+offSize);
	uint32_t cap = (size*1024)/32;

	int* cache = (int*)malloc(sizeof(int)*sets);

  printf("Ways: %u; Sets: %u; Line Size: %uB\n", ways, sets, line);
  printf("Tag: %d bits; Index: %d bits; Offset: %d bits\n", tagSize, indexSize, offSize);

  traceRun(filename ,ways, sets, tagSize, indexSize, cap);
  
  /* TODO: Now we simulate the cache */  
  write_xactions = getWrites();
  totalHits = getHits();
  totalMisses= getMisses();
  /* Print results */
